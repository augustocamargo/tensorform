...
def gammatone(
    ...
    """
    Orchestrates Gammatone filterbank performance profiling over a list of trials.
    ...
    legacy_fn : Callable, optional
        Sequential CPU reference. Defaults to ``GammatoneOperator.legacy_reference``.
    accelerate_fn : Callable, optional
        HAC accelerated callable. Defaults to ``GammatoneOperator.accelerate``.
    iterations : int
    ...
    n_fft : int
        FFT window size (should accommodate the longest filter impulse response).
    hop_length : int
        Analysis hop length in samples.
    n_filters : int
        Number of ERB-spaced gammatone filters.
    f_min : float
