...
def mfcct(
    ...
    """
    Orchestrates MFCCT (Direct Mel Projection + DCT-II) performance profiling.
    ...
    legacy_fn : Callable, optional
        CPU reference. Defaults to ``MFCCTOperator.legacy_reference``.
    accelerate_fn : Callable, optional
        HAC accelerated callable. Defaults to ``MFCCTOperator.accelerate``.
    iterations : int
    ...
    n_fft : int
        Frame length N (paper cepstral default: 1200 = 75 ms at 16 kHz).
    hop_length : int
        Hop size H in samples (paper default: 160 = 10 ms).
    n_mels : int
        Number of Mel projection bins M (paper cepstral default: 128).
    n_mfcct : int
        Number of cepstral coefficients K (paper default: 13).
    f_min : float
        Lowest analysis frequency in Hz (paper cepstral default: 0).
    f_max : float
        Highest analysis frequency in Hz (paper cepstral default: 8000).
    eps : float
